...
func (d *Daemon) fetchK8sLabels(dockerLbls map[string]string) (map[string]string, error) {
	...
	k8sLabels[k8sTypes.PodNamespaceLabelPrefix] = ns
	for k, v := range k8sNSLabels {
		k8sLabels[policy.JoinPath(k8sTypes.PodNamespaceMetaLabelsPrefix, k)] = v
	}
...
func (d *Daemon) getFilteredLabels(allLabels map[string]string) labels.Labels {
	...
	if podName := k8sDockerLbls.GetPodName(allLabels); podName != "" {
		...
		if err != nil {
			...
			k8sSpecialLabels = k8sSpecialLabels.AppendPrefixInKey(k8sTypes.DefaultPolicyParentPathPrefix)
